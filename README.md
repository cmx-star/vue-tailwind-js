...
- ✅ **路由守卫**: 登录状态验证和路由跳转控制
- ✅ **动态菜单**: 基于后端数据动态生成菜单结构
- ✅ **动态导航**: 根据用户权限自动显示/隐藏顶部导航,避免 404
- ✅ **权限标识**: 通过 `permissionValue` 映射页面组件
- ✅ **测试账号**: 内置多个测试账号,快速体验不同权限效果
...
系统内置多个测试账号,用于演示不同权限下的动态导航效果:

| 账号            | 密码       | 角色       | 可见导航                              |
| --------------- | ---------- | ---------- | ------------------------------------- |
| `admin`         | `password` | 超级管理员 | 概览、网络、VPN、边缘计算、向导、系统 |
| `network_admin` | `password` | 网络管理员 | 概览、网络、VPN                       |
| `system_admin`  | `password` | 系统管理员 | 概览、系统                            |
| `user`          | `password` | 普通用户   | 概览                                  |

**使用方法:**

1. 访问登录页面
2. 点击"测试账号"区域的账号按钮,自动填充用户名和密码
3. 点击登录,观察顶部导航栏的变化

**动态导航原理:**

- 根据用户权限返回不同的菜单数据
- `generateTopNavList` action 自动过滤没有菜单的导航项
- 实现真正的动态权限控制,避免显示无权访问的导航
...
│   │   ├── network/    # 网络管理
│   │   ├── system/     # 系统设置
│   │   │   ├── users/  # 用户管理 (ViewUserManagement)
│   │   │   └── roles/  # 角色管理 (ViewRoleManagement)
│   │   ├── vpn/        # VPN 管理
│   │   └── wizard/     # 向导页面
│   ├── App.vue          # 根组件
│   └── main.js          # 入口文件
