...
async function getUniquePageName(viewsRoot) {
  while (true) {
    let rawPageName = await question('请输入页面名称 (例如: Settings, Dashboard): ')
    ...
    if (existingPath) {
      console.error('\x1b[31m%s\x1b[0m', `❌ 错误: 页面名称 "${fileName}" 已在项目中存在！`)
...
async function createPage() {
  ...
  while (true) {
    ...
    const hasSubDir = await question(`是否在 ${currentRelativePath} 下创建子目录？(Y/N, 默认 N): `)
  ...
  if (!fs.existsSync(componentsDir)) {
    ...
    console.log('\x1b[32m%s\x1b[0m', `📦 自动补充 components 目录: ${componentsDir}`)
  }

  // 6. 写入标准 Vue 模板内容 (Vue 3 <script setup>)
...
const title = ref('${pageName.replace('View', '')}')
