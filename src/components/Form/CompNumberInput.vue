  ...
  <div :class="containerClass">
    ...
    <div class="relative flex items-center max-w-[9rem] shadow-sm rounded-lg">
      <button
        ...
        @click="decrement"
      >
        <svg class="w-4 h-4 text-gray-900 dark:text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
export default {
  ...
  computed: {
    ...
    decrementClasses() {
      const disabled = this.disabled || this.value <= this.min ? 'opacity-50 cursor-not-allowed' : '';
      return `${this.baseButtonClasses} rounded-s-lg ${disabled}`;
    },
    incrementClasses() {
      const disabled = this.disabled || this.value >= this.max ? 'opacity-50 cursor-not-allowed' : '';
