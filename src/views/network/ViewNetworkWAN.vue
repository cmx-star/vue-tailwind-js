  ...
  computed: {
    ...
    formItems() {
      return [
        {
          key: 'proto',
          label: this.$t('network.wan.fields.proto'),
          type: 'select',
          options: [
            { label: this.$t('network.wan.options.dhcp'), value: 'dhcp' },
            { label: this.$t('network.wan.options.static'), value: 'static' },
          ],
          required: true,
        },
        {
          key: 'mtu',
          label: this.$t('network.wan.fields.mtu'),
          type: 'input',
          subType: 'number',
        ...
        {
          key: 'ip',
          label: this.$t('network.wan.fields.ip'),
          type: 'input',
        ...
        {
          key: 'netmask',
          label: this.$t('network.wan.fields.netmask'),
          type: 'input',
          show: (form) => form.proto === 'static',
          required: true,
          rules: [
        ...
        {
          key: 'gw',
          label: this.$t('network.wan.fields.gw'),
          type: 'input',
          show: (form) => form.proto === 'static',
          required: true,
          rules: [
        ...
        {
          key: 'dns_mode',
          label: this.$t('network.wan.fields.dnsMode'),
          type: 'select',
          options: (form) => {
            const options = [{ label: this.$t('network.wan.options.manual'), value: 'custom' }]
            if (form.proto === 'dhcp') {
              options.unshift({ label: this.$t('network.wan.options.auto'), value: 'auto' })
            }
            return options
          },
        ...
        {
          key: 'dns1',
          label: this.$t('network.wan.fields.dns1'),
          type: 'input',
          show: (form) =>
        ...
        {
          key: 'dns2',
          label: this.$t('network.wan.fields.dns2'),
          type: 'input',
          show: (form) =>
            form.proto === 'static' || (form.proto === 'dhcp' && form.dns_mode === 'custom'),
          required: true,
  ...
  methods: {
    ...
    async fetchStatus() {
      ...
      this.statusData = setData(mockStatus, wanParams).map((item) => ({
        ...item,
        label: this.$t(`network.wan.status.${item.key}`),
      }))
    },
    handleSubmit() {
